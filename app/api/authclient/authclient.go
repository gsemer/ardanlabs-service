...
// This provides a default client configuration, but it's not recommended
// this is replaced by the user with the application specific settings using
...
// WithClient adds a custom client for processing requests. It's recommended
// not to use the default client and provide your own.
...
func (cln *Client) rawRequest(ctx context.Context, method string, url string, headers map[string]string, r io.Reader, v any) error {
	cln.log(ctx, fmt.Sprintf("rawRequest: started method: %s, url: %s", method, url))
	defer cln.log(ctx, "rawRequest: completed")
	...
	cln.log(ctx, fmt.Sprintf("rawRequest: client do: status: %d", resp.StatusCode))
